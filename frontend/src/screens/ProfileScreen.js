...
const ProfileScreen = ({ history, location }) => {
    ...
    ////useSelector is function that gives acces to entire state(store).and we can pullout orderListMy state from it
    ...
    useEffect(() => {
        if (!userInfo) {//if we dont have user logged
            ...
            if (!user || !user.name) {
                ...
                //dispatch listMyOrders, to get all logged user orders
                dispatch(listMyOrders())//it will get user id from token
                console.log(orders)
            } else {
                setName(user.name)
                setEmail(user.email)
